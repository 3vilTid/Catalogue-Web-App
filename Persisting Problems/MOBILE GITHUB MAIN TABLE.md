...
#### B. Bottom Padding ("fixed bottom bar" or "sticky footer") / Tabs Positioning Incorrect
...
- Does not occur on subsequent visits (e.g., Layer 1 → Main Layer → Layer 1 → Main Layer)
...
Layer 1 → Main Layer → [GLITCH] → [STICKY HEADERS BROKEN] → [BOTTOM PADDING WRONG]

User Flow WITH Orientation Change (once):
Layer 1 → Main Layer → [GLITCH] → [STICKY HEADERS BROKEN] → Change orientation →
[ALL ISSUES FIXED PERMANENTLY]

After Fix:
Layer 1 → Main Layer → Layer 1 → Main Layer → [NO PROBLEMS EVER AGAIN]
```
...
if (isGitHubPages && isMobile && !hasPerformedInitialRerenderFix && currentView === "list") {
  ...
  console.log('🔄 Dispatching resize event to trigger orientation change handler');
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      window.dispatchEvent(new Event('resize'));
      console.log('✅ Resize events dispatched');
    }, 100);
  }, 200);
}
```
...
### What We Know Works
- ✅ The `updateOrientation()` handler (Lines 88-206) works **perfectly**
- ✅ Manual orientation change fixes everything **permanently**
- ✅ Complete table destruction and recreation clears browser cache
- ✅ 100ms delay allows CSS to apply properly

### What We Know About The Bug
- 🐛 Only occurs on **initial load** on GitHub Pages mobile
- 🐛 Does NOT occur on subsequent renders
- 🐛 Does NOT occur on desktop or non-GitHub Pages 
- 🐛 Does NOT occur after orientation change on mobile (even once)
- 🐛 Affects both sticky headers AND bottom bar positioning

### The Mystery
❓ **Why does dispatching the resize event (which triggers the working handler) still not work?**
